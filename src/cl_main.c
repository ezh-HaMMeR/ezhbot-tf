...
#ifdef RENDERER_NULL
void Null_StartFrame(void) {}
void Null_EndFrame(void) {}
#endif

void GFX_Init (void) 
{
#ifdef RENDERER_NULL
    Sys_Printf("GFX_Init: Headless mode, graphics skipped.\n");
    
    // Присваиваем заглушки, чтобы не было падения при вызове
    buffers.StartFrame = Null_StartFrame;
    buffers.EndFrame = Null_EndFrame;
