...
{
	#ifdef RENDERER_NULL
    Sys_Printf("R_Texture_Init: Headless mode, setting null renderer...\n");
    
    // Заглушаем вызовы через лямбда-подобный подход (просто ничего не делаем)
    // Мы не будем создавать отдельные функции, чтобы избежать проблем с линковкой/объявлением
    renderer.TextureLabelSet = (void*)1; // "Заглушка"
    renderer.TextureDelete = (void*)1;
    renderer.TextureInitialiseState = (void*)1;
    
    numgltextures = 2; 
    gltextures[1].texnum = 1; 
    return;
	#else
	int i;
