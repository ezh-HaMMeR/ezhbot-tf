...
{
	#ifdef RENDERER_NULL
    // Полностью пропускаем опрос железа в Headless-режиме
    Sys_Printf("SYSINFO_Init: Headless mode, skipping system info.\n");
    snprintf(f_system_string, sizeof(f_system_string), "Headless Mode");
    return;
	#else	
	LONG  ret;
	HKEY  hKey;
