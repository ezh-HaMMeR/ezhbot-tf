...
void Model_NextDownload (void) 
{
	...
	for (i = 1; i < MAX_MODELS; i++) {
		...
		if (!cl.model_precache[i]) {
			Com_Printf("\n&cf22Couldn't load model:&r %s (ignored for headless)\n", cl.model_name[i]);
		#ifndef RENDERER_NULL
					Host_EndGame();
					return;
		#else
					// В headless-режиме игнорируем отсутствие модели.
					// Подсовываем указатель на саму карту (worldmodel), 
					// чтобы избежать падений (SIGSEGV) при обращении к пустой модели.
					cl.model_precache[i] = cl.model_precache[1]; 
		#endif
